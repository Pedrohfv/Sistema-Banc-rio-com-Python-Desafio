...
def depositar(saldo, valor, extrato):
    if valor > 0:
        ...
        operacao_falhou("O valor informado é inválido.")
    return saldo, extrato


def sacar(saldo, valor, extrato, limite, numero_saques, limite_saques):
    if valor > saldo:
        operacao_falhou("Você não tem saldo suficiente.")
    elif valor > limite:
        operacao_falhou("O valor do saque excede o limite.")
    elif numero_saques >= limite_saques:
        operacao_falhou("Número máximo de saques excedido.")
    elif valor > 0:
        saldo -= valor
...
def criar_usuario(usuarios):
    ...
    if any(usuario['cpf'] == cpf for usuario in usuarios):
        operacao_falhou("Já existe usuário com esse CPF!")
        return
...
def criar_conta(agencia, numero_conta, usuarios):
    ...
    operacao_falhou("Usuário não encontrado, fluxo de criação de conta encerrado!")
